...
def test_parser_active_character():
...
def test_parser_with_level_and_bounded():
...
def test_parser_params_final_hash_p204():
...
def test_parser_params_out_of_order():
...
def test_parser_params_basic_shortargument():
    string = "\\def\\hello#1{a#1b}\\hello 1"
    assert run_code(string,
            find = "chars") =="a1b"

def test_parser_params_basic_longargument():
    string = "\\def\\hello#1{a#1b}\\hello {world}"
    assert run_code(string,
            find = "chars") =="aworldb"

def test_parser_params_with_delimiters():
    string = (
...
def test_parser_params_with_prefix():
    string = (
...
def test_parser_params_non_numeric():
    for forbidden in [
            '!',
            'A',
            r'\q',
...
def test_parser_level():

    PARSER_LEVEL_STRING = (
            r"A \iffalse B\fi C \count20 6 {D} \hbox{E}"
            )

    PARSER_LEVEL_EXPECTED = [
            ('deep', [
...
def test_parser_bounded_with_deep_pushback():
    # Regression test.
...
def test_parser_delegate_simple():
...
def test_parser_delegate_raise():
    doc = Document()
    e = doc.open('ABC', on_eof='none')
...
def test_parser_eat_optional_spaces():
...
def test_parser_invalid_char(caplog):
...
def test_parser_if_in_number():
    with pytest.raises(ValueError):
        run_code(
                r'\catcode`X=13\iffalse8\fi3'
                )

def test_parser_get_digit_sequence():
...
def test_parser_get_digit_sequence_will_stop():
...
def test_parser_step_with_levels():
    ...
    for level, expected in PARSER_STEP_LEVEL_EXPECTED:
        ...
        found = [parser_level_hbox_fix(str(n)) for n in e]
...
PARSER_STEP_LEVEL_EXPECTED = [
        PARSER_LEVEL_EXPECTED[0],
        PARSER_LEVEL_EXPECTED[1],
